...
# Install osxcross
RUN \
    cd /opt/osxcross && ./build_clang.sh UNATTENDED=1 INSTALLPREFIX=/opt/clang && \
    cd /opt/osxcross/build/llvm-3.9.1.src/build_stage2 && make install && \
    cd /opt/osxcross && cp -fv /opt/MacOSX10.11.sdk.tar.xz tarballs/ && \
    cd /opt/osxcross && ./build.sh UNATTENDED=1 PATH=${PATH}:/opt/clang/bin

# Install apple-libtapi
RUN \
    cd /opt/apple-libtapi && ./build.sh INSTALLPREFIX=/opt/osxcross/target PATH=${PATH}:/opt/clang/bin:/opt/osxcross/target/bin && \
